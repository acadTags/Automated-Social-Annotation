...
tf.app.flags.DEFINE_string("mode", "rbf", "SVM mode: linear or rbf")
tf.app.flags.DEFINE_float("C", 0, "parameter C in both svm rbf and linear kernel")
...
def main(_):
    ...
    # training
        ...
        trainY_int = np.asarray(trainY).astype(int);print(trainY_int.shape)
        one_class_label_list=None
        ...
        # #print('trainY_int_pruned:',trainY_int_pruned)

        model = train_cc(trainX_embedded, trainY_int)
        ...
        print(
                    "CC==>Run %d Validation Accuracy: %.3f\tValidation Hamming Loss: %.3f\tValidation Precision: %.3f\tValidation Recall: %.3f\tValidation F-measure: %.3f" % (
        ...
        print("--- validation of fold %s took %s seconds ---" % (num_run, time.time() - start_time_valid))
        ...
        print(
                    "CC==>Run %d Test Accuracy: %.3f\tTest Hamming Loss: %.3f\tTest Precision: %.3f\tTest Recall: %.3f\tTest F-measure: %.3f" % (
    ...
    print(
                "CC==>Final Validation results Validation Accuracy: %.3f ± %.3f (%.3f - %.3f)\tValidation Hamming Loss: %.3f ± %.3f (%.3f - %.3f)\tValidation Precision: %.3f ± %.3f (%.3f - %.3f)\tValidation Recall: %.3f ± %.3f (%.3f - %.3f)\tValidation F-measure: %.3f ± %.3f (%.3f - %.3f)" % (
    ...
    print(
                "CC==>Final Test results Test Accuracy: %.3f ± %.3f (%.3f - %.3f)\tTest Hamming Loss: %.3f ± %.3f (%.3f - %.3f)\tTest Precision: %.3f ± %.3f (%.3f - %.3f)\tTest Recall: %.3f ± %.3f (%.3f - %.3f)\tTest F-measure: %.3f ± %.3f (%.3f - %.3f)" % (
    ...
    output_to_file('svm ' + str(FLAGS.dataset) + " C " + str(FLAGS.C) + ' gamma' + str(FLAGS.gamma) + ' gp_id' + str(
...
def train_svm(trainX_embedded, trainY, mode=FLAGS.mode):
    ...
    if mode == 'rbf':
        model = Meka(
            ...
            weka_classifier = "weka.classifiers.functions.LibSVM -M 5000 -Z -C %f -G %f" % (FLAGS.C, FLAGS.gamma),
            #see https://weka.sourceforge.io/doc.stable/weka/classifiers/functions/LibSVM.html
            meka_classpath="/exports/eddie/scratch/hdong3/scikit_ml_learn_data/meka/meka-release-1.9.2/lib/", # for eddie server
        ...
        model = Meka(
            meka_classifier="meka.classifiers.multilabel.BR",  # Binary Relevance
            weka_classifier = "weka.classifiers.functions.LibLINEAR -C %f" % (FLAGS.C),
        ...
        model = Meka(
            meka_classifier="meka.classifiers.multilabel.BR",  # Binary Relevance
            weka_classifier = "weka.classifiers.functions.LibSVM -M 200 -C %f -G %f" % (FLAGS.C, FLAGS.gamma),
            meka_classpath="/exports/eddie/scratch/hdong3/scikit_ml_learn_data/meka/meka-release-1.9.2/lib/", # for eddie server
            java_command='java'  # path to java executable
        )
...
def train_cc(trainX_embedded, trainY, mode=FLAGS.mode):
    #trainX_embedded_sp = sparse.lil_matrix(trainX_embedded)
    #trainY_sp = sparse.lil_matrix(trainY)
    #print(FLAGS.C,FLAGS.gamma)
    #meka_classpath = download_meka()
...
def do_eval(modelToEval, evalX_embedded, evalY, hamming_q, one_class_label_list=None):
    #evalX_embedded_sp = sparse.lil_matrix(evalX_embedded)
    #if type(evalX_embedded) is not np.ndarray:
