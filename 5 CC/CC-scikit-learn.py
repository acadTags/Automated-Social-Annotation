...
tf.app.flags.DEFINE_integer("num_chains",10,"number of chains to be used")
...
def do_eval_chains(modelChainsToEval, one_class_label_list, evalX_embedded, evalY,hamming_q=FLAGS.ave_labels_per_doc):
    y_pred_chains = np.array([chain.predict(evalX_embedded) for chain in
                          modelChainsToEval])
    y_pred_ensemble = y_pred_chains.mean(axis=0)
    y_pred = y_pred_ensemble >= 0.5
    print(y_pred.shape)
    # restore the original label list with the one_class_label_list (for those labels having no training instances, we assume False in the testing results)
...
def display_for_qualitative_evaluation_chains(modelChainsToEval, one_class_label_list, evalX_embedded, evalX,evalY,vocabulary_index2word,vocabulary_index2word_label):
    prediction_str=""
    #generate the doc indexes same as for the deep learning models.
    number_examples=len(evalY)
    rn_dict={}
