...
tf.app.flags.DEFINE_string("mode", "linear", "SVM mode: linear or rbf [unused for PLST]")
...
tf.app.flags.DEFINE_integer("compressed_matrix_size",3,"Size of the compressed matrix for PLST. Should be less than the number of labels and more than 1. (default: 3)")
...
def main(_):
    ...
    # training
        ...
        model = train_PLST(trainX_embedded, trainY_int_pruned)
        ...
        print(
                    "PLST==>Run %d Validation Accuracy: %.3f\tValidation Hamming Loss: %.3f\tValidation Precision: %.3f\tValidation Recall: %.3f\tValidation F-measure: %.3f" % (
        ...
        print(
                    "PLST==>Run %d Test Accuracy: %.3f\tTest Hamming Loss: %.3f\tTest Precision: %.3f\tTest Recall: %.3f\tTest F-measure: %.3f" % (
    ...
    print(
                "PLST==>Final Validation results Validation Accuracy: %.3f ± %.3f (%.3f - %.3f)\tValidation Hamming Loss: %.3f ± %.3f (%.3f - %.3f)\tValidation Precision: %.3f ± %.3f (%.3f - %.3f)\tValidation Recall: %.3f ± %.3f (%.3f - %.3f)\tValidation F-measure: %.3f ± %.3f (%.3f - %.3f)" % (
    ...
    print(
                "PLST==>Final Test results Test Accuracy: %.3f ± %.3f (%.3f - %.3f)\tTest Hamming Loss: %.3f ± %.3f (%.3f - %.3f)\tTest Precision: %.3f ± %.3f (%.3f - %.3f)\tTest Recall: %.3f ± %.3f (%.3f - %.3f)\tTest F-measure: %.3f ± %.3f (%.3f - %.3f)" % (
...
# train PLST with libSVM as classifier, always using the rbf mode
def train_PLST(trainX_embedded, trainY):
    trainX_embedded_sp = sparse.lil_matrix(trainX_embedded)
    trainY_sp = sparse.lil_matrix(trainY)
    ...
    model = Meka(
        ...
        #meka_classifier = "meka.classifiers.multilabel.CC",
        #meka_classifier = "meka.classifiers.multilabel.MULAN -S HOMER.BalancedClustering.%d.BinaryRelevance" % FLAGS.num_clusters,
        meka_classifier = "meka.classifiers.multilabel.PLST -size %d" % FLAGS.compressed_matrix_size,
        #weka_classifier = "weka.classifiers.functions.Logistic",
        #weka_classifier = "weka.classifiers.functions.SMO -C 1 -K \"weka.classifiers.functions.supportVector.RBFKernel -C 250007 -G 1\"",
        weka_classifier = "weka.classifiers.functions.LibSVM -C %d -G %d" % (FLAGS.C, FLAGS.gamma),
        #weka_classifier = "weka.classifiers.functions.LibSVM",
