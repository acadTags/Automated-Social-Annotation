...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        model=BiGRU(num_classes, FLAGS.learning_rate, FLAGS.batch_size, FLAGS.decay_steps, FLAGS.decay_rate, FLAGS.sequence_length, vocab_size, FLAGS.embed_size, FLAGS.is_training,FLAGS.lambda_sim,FLAGS.lambda_sub,multi_label_flag=FLAGS.multi_label_flag)
        ...
        for train, valid in zip(trainlist, validlist):
            ...
            for epoch in range(curr_epoch,FLAGS.num_epochs):
                ...
                for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)): # might have lost a very little part of data (105 out of 15849) here which is the mod after dividing the batch_size
                    ...
                    curr_summary_str,curr_loss,curr_acc,_=sess.run([model.training_loss,model.loss_val,model.accuracy,model.train_op],feed_dict)
                    ...
                    if FLAGS.weight_decay_testing:                    
                        ...
                        if FLAGS.batch_size!=0 and (start%(FLAGS.validate_step*FLAGS.batch_size)==0): #(epoch % FLAGS.validate_every) or  if epoch % FLAGS.validate_every == 0:
                            ...
                            eval_loss,_,_,_,_,_,_,_,_,_,_ = do_eval_multilabel_threshold(sess,model,label_sim_mat,label_sub_mat,validX,validY,batch_size,vocabulary_index2word,vocabulary_index2word_label,epoch,threshold=0.5,display_results_bool=False,record_to_tensorboard=False) # here we use validation data [not testing data!] # do not record this to tensorboard
