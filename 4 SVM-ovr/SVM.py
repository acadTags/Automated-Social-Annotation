...
tf.app.flags.DEFINE_string("mode","linear","SVM mode: linear or rbf")
...
def main(_):
    ...
    for trainfold in trainlist:
        ...
        trainX = pad_sequences(trainX, maxlen=FLAGS.sequence_length, value=0.)
        if FLAGS.dataset == "sof-sample": 
        # for large dataset which can raise the out of memory error, we separate the data to two parts and preprocess each of them and them combine the results.
            trainX_embedded_1 = get_embedded_words(trainX[0:int(len(trainX)/2)],word_embedding_final,vocab_size)
            print('trainX_embedded_1:',trainX_embedded_1.shape)
            trainX_embedded_2 = get_embedded_words(trainX[int(len(trainX)/2):],word_embedding_final,vocab_size)
            print('trainX_embedded_2:',trainX_embedded_2.shape)
            trainX_embedded = np.concatenate((trainX_embedded_1, trainX_embedded_2), axis=0)
        else:
        ...
        print("SVM==>Run %d Validation Accuracy: %.3f\tValidation Hamming Loss: %.3f\tValidation Precision: %.3f\tValidation Recall: %.3f\tValidation F-measure: %.3f" % (num_run,valid_acc_th[num_run],valid_hamming_loss_th[num_run],valid_prec_th[num_run],valid_rec_th[num_run],valid_fmeasure_th[num_run]))
        output_valid = output_valid + "\n" + "SVM==>Run %d Validation Accuracy: %.3f\tValidation Hamming Loss: %.3f\tValidation Precision: %.3f\tValidation Recall: %.3f\tValidation F-measure: %.3f" % (num_run,valid_acc_th[num_run],valid_hamming_loss_th[num_run],valid_prec_th[num_run],valid_rec_th[num_run],valid_fmeasure_th[num_run]) + "\n" # also output the results of each run.
        ...
        print("SVM==>Run %d Test Accuracy: %.3f\tTest Hamming Loss: %.3f\tTest Precision: %.3f\tTest Recall: %.3f\tTest F-measure: %.3f" % (num_run,test_acc_th[num_run],test_hamming_loss_th[num_run],test_prec_th[num_run],test_rec_th[num_run],test_fmeasure_th[num_run]))
        output_test = output_test + "\n" + "SVM==>Run %d Test Accuracy: %.3f\tTest Hamming Loss: %.3f\tTest Precision: %.3f\tTest Recall: %.3f\tTest F-measure: %.3f" % (num_run,test_acc_th[num_run],test_hamming_loss_th[num_run],test_prec_th[num_run],test_rec_th[num_run],test_fmeasure_th[num_run]) + "\n" # also output the results of each run.
...
def train_svm(trainX_embedded,trainY):
    if FLAGS.mode == 'linear':
        ...
        model = OneVsRestClassifier(SVC()) #everything default, this will be using rbf kernel with default C and gamma settings, see https://scikit-learn.org/stable/modules/generated/sklearn.svm.SVC.html
