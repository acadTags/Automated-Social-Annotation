...
tf.app.flags.DEFINE_integer("batch_size", 1024, "Batch size for training/evaluating.") #批处理的大小 32-->128 #TODO
...
tf.app.flags.DEFINE_string("training_data_path_bib","../datasets/bibsonomy_preprocessed_merged_for_HAN.txt","path of traning data.") # for bibsonomy dataset
...
def main(_):
    ...
    if FLAGS.dataset == "bibsonomy-clean":
        ...
        vocabulary_word2index_label,vocabulary_index2word_label = create_voabulary_label(voabulary_label=traning_data_path, name_scope=FLAGS.dataset + "-HAN") # keep a distinct name scope for each model and each dataset.
        ...
        vocabulary_word2index_label,vocabulary_index2word_label = create_voabulary_label(voabulary_label=traning_data_path, name_scope=FLAGS.dataset + "-HAN")
    ...
    vocabulary_word2index, vocabulary_index2word = create_voabulary(word2vec_model_path,name_scope=FLAGS.dataset + "-HAN")
        ...
        for train, valid in zip(trainlist, validlist):
            ...
            for epoch in range(curr_epoch,FLAGS.num_epochs):
                ...
                for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)): # might have lost a very little part of data (105 out of 15849) here which is the mod after dividing the batch_size
                    ...
                    curr_summary_str,curr_loss,curr_acc,_=sess.run([model.training_loss,model.loss_val,model.accuracy,model.train_op],feed_dict)#curr_acc--->modelToEval.accuracy
                    ...
                    if FLAGS.weight_decay_testing:                    
                        ...
                        if FLAGS.batch_size!=0 and (start%(FLAGS.validate_step*FLAGS.batch_size)==0):
                            ...
                            eval_loss,_,_,_,_,_,_,_,_,_,_ = do_eval_multilabel_threshold(sess,model,label_sim_mat,label_sub_mat,validX,validY,batch_size,vocabulary_index2word,vocabulary_index2word_label,epoch,threshold=0.5,display_results_bool=False,record_to_tensorboard=False)
...
def do_eval_multilabel_threshold(sess,modelToEval,label_sim_mat,label_sub_mat,evalX,evalY,batch_size,vocabulary_index2word,vocabulary_index2word_label,epoch,threshold=0.5,display_results_bool=True,hamming_q=FLAGS.ave_labels_per_doc,top_number=FLAGS.topk,record_to_tensorboard=True):
    ...
    for start,end in zip(range(0,number_examples,batch_size),range(batch_size,number_examples,batch_size)):
        ...
        curr_summary_str,curr_eval_loss,logits= sess.run([modelToEval.validation_loss,modelToEval.loss_val,modelToEval.logits],feed_dict)#curr_eval_acc--->modelToEval.accuracy
